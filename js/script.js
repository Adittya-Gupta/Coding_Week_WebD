...
searchinput.addEventListener('input', e => {
  ...
  list.forEach(object => {
    const isVisible = object.title.toLowerCase().includes(value.toLowerCase()) || object.description.toLowerCase().includes(value.toLowerCase())
    object.element.classList.toggle("hide", !isVisible)
  })
})
//I have pasted all the data in the js file as was asked to do in the #doubts channel in cc discord
const data = {
  "products": [
    {
      "id": 1,
...
list = data.products.map(product => {
  ...
  const discount = card.querySelector("[product-disc]")
  ...
  stock.textContent = "Order now only " + product.stock + " left!!!"
